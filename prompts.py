"""Gemini AI使いこなし術 - プロンプト定義

Google Gemini特化ブログ用のプロンプトを一元管理する。
JSON-LD構造化データ（BlogPosting / FAQPage / BreadcrumbList）対応。
"""

# ペルソナ設定
PERSONA = (
    "あなたはGemini AIの日本語エキスパートです。"
    "Google Workspace統合やAndroid連携に精通し、"
    "初心者から開発者まで幅広い読者に実践的な情報を届けるプロのテックライターです。"
    "Geminiの最新アップデートを常にキャッチアップし、"
    "他のAI（ChatGPT、Claude等）との比較も客観的に行えます。"
)
...
## Google連携活用テクニック
（Gmail / Docs / Sheets / Maps / Android との連携方法）

## 他のAIとの比較
（ChatGPT / Claude / Copilot との違いを表形式で整理）
...
# カテゴリ別SEOキーワードヒント
CATEGORY_PROMPTS = {
    "Gemini 使い方": "Gemini 使い方、Gemini 始め方、Gemini 初心者、Gemini 無料、Gemini チャット",
    "Gemini 料金・プラン": "Gemini 料金、Gemini 無料 有料 違い、Google One AI Premium、Gemini 月額",
    "Gemini Advanced": "Gemini Advanced、Gemini 2.5 Pro、Gemini Advanced 機能、Gemini Advanced レビュー",
    "Gemini vs ChatGPT": "Gemini ChatGPT 比較、Gemini ChatGPT 違い、AI 比較 2026、どっちがいい",
    "Gemini API・開発": "Gemini API、Gemini API 使い方、Gemini SDK、Gemini 開発、Google AI Studio",
    "Gemini 最新ニュース": "Gemini アップデート、Gemini 新機能、Google AI 最新、Gemini リリース",
    "Gemini × Google連携": "Gemini Gmail、Gemini Google Docs、Gemini Workspace、Gemini Android",
    "Gemini 活用事例": "Gemini ビジネス活用、Gemini 仕事効率化、Gemini 使い道、AI 業務活用",
}
...
NEWS_SOURCES = [
    ...
    "TechCrunch (https://techcrunch.com/tag/gemini/)",
...
KEYWORD_PROMPT_EXTRA = (
    "Google Gemini AIに関するキーワードを選んでください。\n"
    "日本のユーザーが検索しそうな実用的なキーワードを意識してください。\n"
    "「Gemini 使い方」「Gemini 料金」「Gemini vs ChatGPT」のような、\n"
...
def build_keyword_prompt(config):
    ...
    return (
        f"{PERSONA}\n\n"
        "Gemini AI使いこなし術ブログ用のキーワードを選定してください。\n\n"
...
- 2026年最新の情報を反映すること
- 具体的な操作手順や設定方法を含める
- Google連携（Gmail, Docs, Workspace等）の活用テクニックを含める
- 他AIとの客観的な比較を含める
